...
- **Presentation** — `@MainActor` ViewModels adopting `ObservableObject`, nested `ViewState` structs, feature folder slices, lightweight composition (`TaskExplorerComposition` / `TaskExplorerEnvironment`) instead of a third-party DI container.
...
- **Tools** — IDE-integrated LLM assistance (coding assistant).  
...
- **Manual decisions** — Layered folders (domain / data / core / presentation), persistence strategy (local overrides map), UX for loading/error/refresh parity, deliberately avoiding extra SPM dependencies so the Xcode project stays self-contained.  
